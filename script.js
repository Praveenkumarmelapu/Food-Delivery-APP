...
function getLocation() {
    ...
    if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition((position) => {
            setTimeout(() => {
                locationText.textContent = "New York, USA (Detected)";
...
function openProfile(tabName = 'profile') {
...
function switchProfileTab(tabName) {
    ...
    // Find the button that called this or corresponds to the tab
    const activeBtn = Array.from(btns).find(b => b.textContent.toLowerCase() === tabName);
